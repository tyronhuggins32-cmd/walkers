(() => {
  ...
  function createProfilePicker() {
    ...
    if (version) version.textContent = "BUILD 4.0";
  ...
  function createGameUi(game) {
    ...
    if (actions && !document.getElementById("hcHealthBtn")) {
      const health = makeButton(
        "hcHealthBtn",
        "HEALTH H",
        "Open body condition (H)"
      );

      const skills = makeButton(
        "hcSkillsBtn",
        "SKILLS K",
        "Open skills (K)"
      );
    ...
    createHealthPanel(root, game);
    createSkillsPanel(root, game);
  ...
  function createHealthPanel(root, game) {
    ...
    panel.className = "hc-deep-panel hidden";
    panel.innerHTML = `
      <header>
        <div><p>BODY CONDITION</p><h2>Health & treatment</h2></div>
        <button class="hc-panel-close" type="button">×</button>
      </header>
      <div class="hc-panel-scroll">
    ...
    panel.querySelector(".hc-panel-close")
  ...
  function createSkillsPanel(root, game) {
    ...
    panel.innerHTML = `
      <header>
        <div><p>SURVIVOR DEVELOPMENT</p><h2>Skills & equipment</h2></div>
        <button class="hc-panel-close" type="button">×</button>
      </header>
      <div class="hc-panel-scroll">
        ...
        <div class="hc-profile-card">
          <b>H Health · K Skills · V Vehicle · F Forage · Q Shout</b>
  ...
  function toggleCustomPanel(game, id) {
    ...
    state.customPanel = id;
    game.panelOpen = id;
    game.gameScreen?.classList.add("panel-open");
  ...
  function closeCustomPanel(game) {
    document.getElementById("hcHealthPanel")
      ?.classList.add("hidden");

    document.getElementById("hcSkillsPanel")
      ?.classList.add("hidden");

    if (
      game &&
      ["hcHealthPanel", "hcSkillsPanel"].includes(
        game.panelOpen
      )
    ) {
      game.panelOpen = null;
      game.gameScreen?.classList.remove("panel-open");
    }
  ...
  function updateDeepUi(game) {
    ...
    if (state.customPanel === "hcHealthPanel") {
      renderHealthPanel(game);
    } else if (state.customPanel === "hcSkillsPanel") {
      renderSkillsPanel(game);
    }
  }
