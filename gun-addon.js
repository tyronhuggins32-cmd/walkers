(() => {
  ...
  const ANIMATIONS = Object.freeze({
    pistol: { recoil: 3, rise: 0.035, shake: 0.1, cycle: "slide", scale: 0.78 },
    revolver: { recoil: 5.5, rise: 0.085, shake: 0.18, cycle: "hammer", scale: 0.8 },
    machine_pistol: { recoil: 2.8, rise: 0.045, shake: 0.8, cycle: "slide", scale: 0.77 },
    smg: { recoil: 3.2, rise: 0.032, shake: 0.9, cycle: "bolt", scale: 0.75 },
    shotgun: { recoil: 7, rise: 0.075, shake: 0.25, cycle: "pump", scale: 0.76 },
    double_barrel: { recoil: 9, rise: 0.11, shake: 0.3, cycle: "break", scale: 0.77 },
    rifle: { recoil: 7.5, rise: 0.07, shake: 0.15, cycle: "bolt", scale: 0.75 },
    carbine: { recoil: 4.2, rise: 0.04, shake: 0.55, cycle: "bolt", scale: 0.75 },
    assault_rifle: { recoil: 4.6, rise: 0.045, shake: 0.75, cycle: "bolt", scale: 0.75 },
    lever_rifle: { recoil: 6.5, rise: 0.065, shake: 0.18, cycle: "lever", scale: 0.76 },
    suppressed_pistol: { recoil: 2.1, rise: 0.022, shake: 0.06, cycle: "slide", scale: 0.78 },
    hand_cannon: { recoil: 10.5, rise: 0.14, shake: 0.35, cycle: "heavy-slide", scale: 0.8 },
    auto_shotgun: { recoil: 6.2, rise: 0.06, shake: 0.65, cycle: "gas", scale: 0.76 },
    sniper_rifle: { recoil: 9, rise: 0.09, shake: 0.12, cycle: "sniper-bolt", scale: 0.75 },
    lmg: { recoil: 4.8, rise: 0.035, shake: 1.15, cycle: "belt", scale: 0.74 }
  });
  ...
  function drawGun(
    ...
    const dark = "#07090a";
    const metal = ctx.createLinearGradient(0, -8, 0, 9);
    metal.addColorStop(0, "#596166");
    metal.addColorStop(0.28, "#343b3f");
    metal.addColorStop(0.72, "#20272a");
    metal.addColorStop(1, "#121719");
    const metal2 = ctx.createLinearGradient(0, -7, 0, 10);
    metal2.addColorStop(0, "#879095");
    metal2.addColorStop(0.45, "#535c60");
    metal2.addColorStop(1, "#252c2f");
    const shine = "#aeb7b8";
    const wood = ctx.createLinearGradient(0, -8, 0, 10);
    wood.addColorStop(0, "#9a6844");
    wood.addColorStop(0.5, "#70472e");
    wood.addColorStop(1, "#3e271d");
    const wood2 = "#b27a50";
    ...
    function box(
      ...
      const radius = Math.min(2.4, Math.max(0.8, height * 0.28));
    ...
    function circle(px, py, radius, color) {
      ctx.fillStyle = color;
      ctx.beginPath();
    ...
    /* A crisp detail pass keeps every firearm readable at gameplay scale. */
    ctx.shadowBlur = 0;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;

    const drawTriggerGuard = (px, py, radiusX = 4, radiusY = 3) => {
      ctx.strokeStyle = "#111618";
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.ellipse(px, py, radiusX, radiusY, 0, 0, Math.PI);
      ctx.stroke();
      line(px, py - 1, px + 1.5, py + 2, 1, "#899194");
    };

    const gripTexture = (px, py, width, count = 3) => {
    ...
    switch (gun.model) {
      case "pistol":
        rect(4 - mechanical * 4, -3, 7, 3, "#161c1f");
        for (let x = -2; x <= 3; x += 2) {
          line(x - mechanical * 4, -4, x - 1 - mechanical * 4, 0, 0.8, "#aab2b3");
        }
        gripTexture(1, 5, 4, 3);
        drawTriggerGuard(8, 4, 3.6, 2.4);
        rect(13 - mechanical * 4, -7, 2, 2, "#d0d5d2");
        rect(-3 - mechanical * 4, -7, 3, 2, "#8c9698");
        circle(17 - mechanical * 4, -1.5, 1.5, "#050708");
        break;

      case "revolver":
        line(7, -5, 17, -5, 1.2, "#a6afb0");
        circle(1, 1, 1.2, "#bbc1be");
        drawTriggerGuard(7, 4, 4.2, 2.8);
        gripTexture(-4, 7, 5, 3);
        rect(12, -6, 2, 2, "#d0d5d2");
        circle(17, -1, 1.6, "#050708");
        break;

      case "machine_pistol":
        rect(5 - mechanical * 5, -3, 8, 3, "#111719");
        for (let y = 6; y < 16; y += 3) {
          line(8, y, 11, y + 1, 0.8, "#70797c");
        }
        gripTexture(0, 6, 4, 3);
        drawTriggerGuard(8, 4, 3.8, 2.4);
        rect(14 - mechanical * 5, -7, 2, 2, "#c7ceca");
        circle(19 - mechanical * 5, -1, 1.5, "#050708");
        break;

      case "smg":
        line(-24, -3, -31, -8, 2.2, "#2e3739");
        line(-24, 2, -31, 7, 2.2, "#2e3739");
        line(-31, -8, -31, 7, 2.2, "#70797b");
        for (let x = 12; x <= 27; x += 5) circle(x, 0, 1.2, "#090d0e");
        gripTexture(-1, 7, 4, 4);
        drawTriggerGuard(9, 5, 4, 2.8);
        box(3, -11, 11, 4, "#151b1d");
        circle(10, -9, 1.4, "#6da9aa");
        circle(35, -0.5, 1.8, "#050708");
        break;

      case "shotgun":
        for (let x = 14 - mechanical * 12; x < 25 - mechanical * 12; x += 3) {
          line(x, 4, x - 1, 9, 0.8, "rgba(238,203,151,.34)");
        }
        line(-19, 0, -8, 0, 1, "rgba(238,203,151,.32)");
        drawTriggerGuard(3, 5, 4.5, 3);
        rect(43, -5, 2, 2, "#d8c67b");
        circle(47, -0.5, 1.9, "#050708");
        break;

      case "double_barrel":
        line(-19, 0, -8, 0, 1, "rgba(238,203,151,.32)");
        for (let x = -2; x < 8; x += 3) circle(x, 0, 0.9, "#8c7658");
        drawTriggerGuard(2, 6, 5, 3.5);
        circle(49 - mechanical * 3, -3 + mechanical * 4, 1.8, "#050708");
        circle(49 - mechanical * 3, 3 + mechanical * 4, 1.8, "#050708");
        break;

      case "rifle":
        line(-21, 0, -9, 0, 1, "rgba(238,203,151,.32)");
        rect(4, -9, 18, 2, "#555f63");
        circle(5, -8, 2, "#7fa6ac");
        circle(22, -8, 2, "#28373b");
        drawTriggerGuard(4, 5, 4.5, 3);
        circle(13 - mechanical * 9, -5, 1.5, "#d1d7d4");
        line(14 - mechanical * 9, -4, 18 - mechanical * 9, -1, 1.4, "#707a7d");
        rect(56, -5, 4, 2, "#a5adae");
        circle(62, -0.5, 1.8, "#050708");
        break;

      case "carbine":
        polygon([[-22, -4], [-29, -8], [-29, 7], [-22, 2]], "#30393c");
        line(-27, -6, -27, 5, 1, "#8b9495");
        for (let x = 24; x < 44; x += 5) circle(x, 0, 1.1, "#080b0c");
        for (let y = 7; y < 16; y += 3) line(10, y, 14, y + 1, 0.8, "#70797c");
        drawTriggerGuard(7, 5, 4, 2.7);
        box(2, -12, 12, 4, "#1b2224");
        circle(10, -10, 1.5, "#74a9a6");
        circle(51, -0.5, 1.8, "#050708");
        break;

      case "assault_rifle":
        for (let x = 28; x < 52; x += 5) circle(x, 0, 1.1, "#080b0c");
        for (let y = 7; y < 16; y += 3) line(11, y, 15, y + 1, 0.8, "#8c7453");
        line(-20, -1, -8, -1, 1, "rgba(239,204,151,.3)");
        drawTriggerGuard(7, 5, 4, 2.7);
        line(-5, -8, 22, -8, 1.6, "#687174");
        for (let x = -3; x <= 20; x += 4) rect(x, -9, 2, 2, "#9ba3a4");
        circle(59, -0.5, 1.8, "#050708");
        break;

      case "lever_rifle":
        line(-21, 0, -8, 0, 1, "rgba(238,203,151,.34)");
        line(21, 4, 37, 4, 1, "rgba(238,203,151,.28)");
        drawTriggerGuard(5, 5, 4, 2.7);
        rect(56, -5, 3, 2, "#d7c983");
        circle(61, -0.5, 1.8, "#050708");
        break;

      case "suppressed_pistol":
        for (let x = 19; x < 38; x += 4) line(x, -4, x, 1, 0.8, "rgba(137,190,176,.28)");
        rect(4 - mechanical * 4, -3, 8, 3, "#142123");
        gripTexture(1, 5, 4, 3);
        drawTriggerGuard(8, 4, 3.6, 2.4);
        circle(40, -1.5, 2, "#050708");
        break;

      case "hand_cannon":
        for (let x = 14 - mechanical * 7; x <= 25 - mechanical * 7; x += 4) {
          rect(x, -7, 2, 3, "#0b0e0f");
        }
        rect(7 - mechanical * 7, -4, 11, 4, "#15191b");
        gripTexture(-1, 7, 7, 4);
        drawTriggerGuard(11, 5, 5, 3.2);
        circle(32 - mechanical * 7, -1.5, 2.2, "#050708");
        break;

      case "auto_shotgun":
        for (let x = 26; x < 50; x += 5) circle(x, 0, 1.2, "#090b0d");
        for (let y = 7; y < 18; y += 3) line(10, y, 15, y + 1, 0.8, "#81708b");
        drawTriggerGuard(7, 5, 4.2, 2.8);
        line(-6, -9, 21, -9, 1.5, "#7c8588");
        box(2, -13, 12, 4, "#24252d");
        circle(10, -11, 1.5, "#a17abd");
        circle(55, -0.5, 1.9, "#050708");
        break;

      case "sniper_rifle":
        line(-25, 0, -9, 0, 1, "rgba(183,207,220,.28)");
        circle(3, -9.5, 2.5, "#4f7884");
        circle(28, -9.5, 2.5, "#243239");
        line(5, -9.5, 26, -9.5, 1.2, "#9caeb7");
        drawTriggerGuard(5, 5, 4.2, 2.8);
        line(43, 3, 39, 16, 1.5, "#6b777d");
        line(57, 3, 62, 16, 1.5, "#6b777d");
        for (let x = 66; x <= 71; x += 3) rect(x, -5, 1, 9, "#0b1012");
        circle(73, -0.5, 2, "#050708");
        break;

      case "lmg":
        for (let x = 32; x < 60; x += 5) circle(x, 0, 1.2, "#090c0b");
        for (let x = 10; x < 23; x += 3) {
          rect(x, 8, 2, 3, "#c9ad52");
        }
        drawTriggerGuard(7, 6, 4.5, 3);
        circle(25, -10, 1.5, "#a9a052");
        line(43, 3, 39, 18, 1.7, "#717b77");
        line(58, 3, 63, 18, 1.7, "#717b77");
        circle(67, -0.5, 2, "#050708");
        break;
    }
  ...
  function installGunModels(game) {
    ...
    /*
     * The articulated character renderer uses the exact same firearm models
     * for hand placement and inventory previews. Keeping one renderer avoids
     * a second, mismatched set of gun artwork.
     */
    window.__hcDrawDetailedGunModelV5 = drawGun;
    window.__hcGunModelAnimationsV5 = ANIMATIONS;
