(() => {
  ...
  var ITEMS = Object.freeze({
    ...
    suppressed_pistol: { name: "Suppressed 9 mm pistol", icon: "SUP", kind: "weapon", mode: "ranged", gunType: "suppressed_pistol", damage: 34, range: 590, cooldown: 0.27, noise: 175, spread: 0.022, ammo: "ammo9", weight: 1.25, maxStack: 1, description: "A threaded sidearm with a worn suppressor. Quiet does not mean silent." },
    hand_cannon: { name: ".50 hand cannon", icon: "50", kind: "weapon", mode: "ranged", gunType: "hand_cannon", damage: 92, range: 720, cooldown: 0.72, noise: 940, spread: 0.026, ammo: "magnum_round", weight: 2, maxStack: 1, description: "Massive recoil, limited capacity, and enough force to fold a walker." },
    auto_shotgun: { name: "Automatic shotgun", icon: "A12", kind: "weapon", mode: "ranged", gunType: "auto_shotgun", damage: 27, pellets: 7, spread: 0.16, range: 430, cooldown: 0.48, noise: 840, ammo: "shell", automatic: true, weight: 4.2, maxStack: 1, description: "A gas-operated tactical shotgun that clears crowds quickly." },
    sniper_rifle: { name: "Precision sniper rifle", icon: "SNP", kind: "weapon", mode: "ranged", gunType: "sniper_rifle", damage: 118, range: 1100, cooldown: 1.18, noise: 1050, spread: 0.006, ammo: "rifle_round", weight: 5.1, maxStack: 1, description: "Long, heavy, and brutally accurate across open streets." },
    lmg: { name: "5.56 light machine gun", icon: "LMG", kind: "weapon", mode: "ranged", gunType: "lmg", damage: 31, range: 720, cooldown: 0.095, noise: 810, spread: 0.052, ammo: "ammo556", automatic: true, weight: 7.4, maxStack: 1, description: "A belt-fed support gun with a deep magazine and violent sustained recoil." },
    ...
    magnum_round: { name: ".50 magnum rounds", icon: "50", kind: "ammo", weight: 0.075, maxStack: 14, description: "Rare oversized cartridges for the hand cannon." },
...
if (upstairs) {
  ...
  (() => {
    ...
    const WEAPONS = Object.freeze({
      ...
      suppressed_pistol: { name: "suppressed pistol", mode: "ranged", damage: 34, range: 590, cooldown: 0.27, noise: 175 },
      hand_cannon: { name: "hand cannon", mode: "ranged", damage: 92, range: 720, cooldown: 0.72, noise: 940 },
      auto_shotgun: { name: "automatic shotgun", mode: "ranged", damage: 58, range: 430, cooldown: 0.48, noise: 840 },
      sniper_rifle: { name: "sniper rifle", mode: "ranged", damage: 118, range: 1100, cooldown: 1.18, noise: 1050 },
      lmg: { name: "light machine gun", mode: "ranged", damage: 31, range: 720, cooldown: 0.095, noise: 810 }
    });
    ...
    const RANGED = /* @__PURE__ */ new Set(["pistol", "revolver", "machine_pistol", "smg", "shotgun", "double_barrel", "rifle", "carbine", "assault_rifle", "lever_rifle", "suppressed_pistol", "hand_cannon", "auto_shotgun", "sniper_rifle", "lmg"]);
  ...
  (() => {
    ...
    const WEAPONS = Object.freeze({
      ...
      suppressed_pistol: { mode: "ranged" },
      hand_cannon: { mode: "ranged" },
      auto_shotgun: { mode: "ranged" },
      sniper_rifle: { mode: "ranged" },
      lmg: { mode: "ranged" }
    });
    const LONG_GUNS = /* @__PURE__ */ new Set(["smg", "shotgun", "double_barrel", "rifle", "carbine", "assault_rifle", "lever_rifle", "auto_shotgun", "sniper_rifle", "lmg"]);
    ...
    function drawGun(ctx, id, x, y, dx, dy, firing = false) {
      if (window.__hollowCountyDetailedGunModelsV5) return;
      const angle = Math.atan2(dy, dx);
    ...
    function drawMelee(ctx, id, x, y, dx, dy) {
      ...
      const wood = ctx.createLinearGradient(0, -5, 0, 5);
      wood.addColorStop(0, "#a7734d");
      wood.addColorStop(0.5, "#70472f");
      wood.addColorStop(1, "#3f281e");
      const steel = ctx.createLinearGradient(0, -7, 0, 7);
      steel.addColorStop(0, "#e0e5e2");
      steel.addColorStop(0.28, "#aab4b3");
      steel.addColorStop(0.7, "#626d6f");
      steel.addColorStop(1, "#343c3e");
      ...
      const rounded = (px, py, width, height, radius, color) => {
        ctx.fillStyle = color;
      ...
      switch (id) {
        case "knife":
          rounded(-8, -3.5, 13, 7, 2.2, "#24201d");
          rounded(-6.5, -2.3, 10, 4.6, 1.5, "#6f4934");
          for (let grip = -4; grip <= 1; grip += 2.5) {
            line(grip, -2, grip - 1, 2, 0.7, "rgba(225,202,166,.34)");
          }
          rivet(-3, 0, 0.9);
          line(4, -4.5, 4, 4.5, 2, "#454d4d");
          polygon([[5, -3.1], [length - 2, -2.4], [length + 3, 0], [length - 1, 2.2], [5, 2.6]], steel);
          line(7, -1.7, length - 1, -1.2, 0.85, "rgba(255,255,245,.7)");
          break;

        case "hammer":
          line(-8, 0, length - 3, 0, 6.5, "#2a1d17");
          line(-7, -0.4, length - 3, -0.4, 4.2, wood);
          rounded(-8, -3, 8, 6, 2, "#29231f");
          for (let grip = -6; grip <= -1; grip += 2) line(grip, -2, grip - 1, 2, 0.7, "#80634e");
          rounded(length - 7, -7, 11, 14, 2.3, steel);
          rounded(length + 2, -5, 5, 10, 1.4, "#778285");
          polygon([[length - 7, -5], [length - 14, -8], [length - 11, -1], [length - 7, 1]], "#626c6e");
          line(length - 12, -5, length - 8, -3, 1, "#d2d8d5");
          break;
        ...
        case "bat":
          ...
          ctx.beginPath();
          ctx.moveTo(-8, -3.4);
          ctx.bezierCurveTo(2, -4, length - 9, -6.2, length + 1, -4.4);
          ctx.quadraticCurveTo(length + 5, 0, length + 1, 4.4);
          ctx.bezierCurveTo(length - 9, 6.2, 2, 4, -8, 3.4);
          ctx.closePath();
          ctx.fill();
          ctx.fillStyle = wood;
          ctx.beginPath();
          ctx.moveTo(-7, -2.3);
          ctx.bezierCurveTo(3, -2.8, length - 9, -4.7, length, -3.3);
          ctx.quadraticCurveTo(length + 3, 0, length, 3.3);
          ctx.bezierCurveTo(length - 9, 4.7, 3, 2.8, -7, 2.3);
          ctx.closePath();
          ctx.fill();
          rounded(-10, -4, 4, 8, 1.4, "#25201c");
          for (let grip = -6; grip <= 3; grip += 2.5) line(grip, -2.6, grip - 1, 2.6, 0.8, "#b5a28a");
          line(6, -2, length - 3, -3, 1, "rgba(235,198,145,.28)");
          break;

        case "axe":
          line(-8, 0, length - 1, 0, 7, "#2b1e17");
          line(-7, -0.5, length - 1, -0.5, 4.5, wood);
          rounded(-9, -3.5, 9, 7, 2, "#27211d");
          for (let grip = -7; grip <= -1; grip += 2.2) line(grip, -2.4, grip - 1, 2.4, 0.75, "#9a765a");
          polygon([[length - 7, -9], [length + 4, -7], [length + 8, -1], [length + 4, 7], [length - 7, 9], [length - 4, 0]], "#8f302d");
          polygon([[length + 3, -7], [length + 9, -2], [length + 9, 2], [length + 3, 7], [length + 5, 1], [length + 5, -1]], steel);
          line(length - 4, -6, length + 2, -5, 1, "rgba(255,210,185,.42)");
          break;

        case "machete":
          rounded(-9, -4, 15, 8, 2.2, "#231f1c");
          rounded(-7, -2.8, 11, 5.6, 1.5, "#51463b");
          for (let grip = -6; grip <= 2; grip += 2.5) line(grip, -2.4, grip - 1, 2.4, 0.9, "#897d6c");
          rivet(-4, 0, 0.9);
          rivet(1, 0, 0.8);
          line(5, -5, 5, 5, 2, "#4c5455");
          ctx.fillStyle = steel;
          ctx.beginPath();
          ctx.moveTo(6, -3.5);
          ctx.quadraticCurveTo(length - 7, -7, length + 3, -3.2);
          ctx.quadraticCurveTo(length + 6, 0, length + 1, 3.8);
          ctx.lineTo(6, 2.4);
          ctx.closePath();
          ctx.fill();
          line(9, -2, length - 2, -3, 0.9, "rgba(255,255,245,.62)");
          break;

        case "katana":
          rounded(-11, -3.3, 18, 6.6, 1.8, "#17191a");
          for (let grip = -9; grip <= 4; grip += 3) {
            line(grip, -2.8, grip + 2.2, 2.8, 1, "#8e3f39");
            line(grip + 2.2, -2.8, grip, 2.8, 0.7, "#b8a26e");
          }
          rounded(-12, -4, 3, 8, 1, "#25292a");
          rounded(6, -6, 3, 12, 1.2, "#b19b55");
          line(8, 0, length, -1.2, 6.2, "#343b3d");
          line(8, -0.5, length, -1.7, 4.4, steel);
          line(10, -2.1, length - 1, -3, 0.9, "rgba(255,255,250,.78)");
          polygon([[length - 2, -4], [length + 5, -1.7], [length - 1, 2]], steel);
          break;

        case "crowbar":
          ctx.strokeStyle = "#351817";
          ctx.lineWidth = 7;
          ctx.lineCap = "round";
          ctx.beginPath();
          ctx.moveTo(-8, 5);
          ctx.quadraticCurveTo(-11, -1, -5, -3);
          ctx.lineTo(length - 4, -1);
          ctx.quadraticCurveTo(length + 4, -1, length + 3, -8);
          ctx.stroke();
          ctx.strokeStyle = "#a6413b";
          ctx.lineWidth = 4.2;
          ctx.beginPath();
          ctx.moveTo(-8, 5);
          ctx.quadraticCurveTo(-11, -1, -5, -3);
          ...
          ctx.quadraticCurveTo(length + 4, -1, length + 3, -8);
          ctx.stroke();
          line(-9, 5, -3, 5, 3.5, "#c25c50");
          line(length + 3, -8, length + 8, -6, 3.5, "#c25c50");
          line(2, -3, length - 4, -1, 0.8, "rgba(255,183,161,.34)");
          break;

        case "spear":
          line(-12, 0, length - 2, 0, 7, "#2b1e17");
          line(-11, -0.5, length - 2, -0.5, 4.4, wood);
          rounded(-13, -3.5, 10, 7, 2, "#29231f");
          for (let wrap = length - 9; wrap <= length - 2; wrap += 2) line(wrap, -3, wrap - 1, 3, 1, "#9a7757");
          polygon([[length - 2, -6], [length + 10, 0], [length - 2, 6], [length + 1, 0]], steel);
          line(length, -3.5, length + 7, 0, 0.9, "rgba(255,255,245,.78)");
          break;

        case "sledgehammer":
          line(-10, 0, length - 2, 0, 8, "#2b1e17");
          line(-9, -0.6, length - 2, -0.6, 5, wood);
          rounded(-11, -4, 12, 8, 2.2, "#24201d");
          for (let grip = -9; grip <= -1; grip += 2.6) line(grip, -3, grip - 1, 3, 0.8, "#8f7057");
          rounded(length - 8, -10, 15, 20, 2.5, "#252c2e");
          rounded(length - 7, -9, 13, 18, 2, steel);
          rounded(length - 10, -8, 5, 16, 1.5, "#596365");
          rounded(length + 4, -8, 5, 16, 1.5, "#727d7f");
          line(length - 4, -7, length + 3, -7, 1, "rgba(255,255,245,.5)");
          break;

        default:
          line(-7, 0, length, 0, 6, "#29211b");
          line(-6, -0.4, length - 1, -0.4, 4, wood);
          break;
      }
      ctx.restore();
    }
  ...
  (() => {
    ...
    const WEAPONS = Object.freeze({
      ...
      suppressed_pistol: ["ranged", 34, 590],
      hand_cannon: ["ranged", 92, 720],
      auto_shotgun: ["ranged", 58, 430],
      sniper_rifle: ["ranged", 118, 1100],
      lmg: ["ranged", 31, 720]
    });
    const clamp2 = (value, min, max) => Math.max(min, Math.min(max, value));
    const distance2 = (a, b) => Math.hypot(a.x - b.x, a.y - b.y);
