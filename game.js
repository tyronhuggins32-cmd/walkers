    ...
    drawCars(ctx, shakeX, shakeY) {
      ...
      for (const building of this.world.buildings) {
        for (const prop of building.decor || []) {
          ...
          const kind = prop.kind;
          const variant = prop.variant || 0;
          const tone = prop.tone || 0;
          const cloth = ["#6f6653", "#6a584b", "#59675c", "#5d5361"][variant % 4];
          const wood = ["#67513d", "#725a42", "#5e4938", "#7a6044"][variant % 4];
          const metal = ["#58615d", "#68716d", "#4f5a57", "#737a74"][variant % 4];
          ...
          if (!INTERIOR_DECOR_FLAT.has(kind)) {
            ctx.fillStyle = "rgba(3,5,4,.3)";
            ctx.beginPath();
            ctx.ellipse(3, 5, INTERIOR_DECOR_LARGE.has(kind) ? 18 : 11, INTERIOR_DECOR_LARGE.has(kind) ? 7 : 5, 0, 0, Math.PI * 2);
            ctx.fill();
          }

          if (["rug", "blanket", "linen", "tarp", "fallen_flag"].includes(kind)) {
            const w = kind === "rug" ? 42 : kind === "tarp" ? 38 : 30;
            const h = kind === "rug" ? 27 : 20;
            ctx.fillStyle = kind === "fallen_flag" ? "#5f4944" : cloth;
            roundedRectPath(ctx, -w / 2, -h / 2, w, h, 3);
            ctx.fill();
            ctx.strokeStyle = "rgba(29,24,20,.45)";
            ctx.strokeRect(-w / 2 + 1.5, -h / 2 + 1.5, w - 3, h - 3);
            ctx.fillStyle = "rgba(230,219,184,.09)";
            for (let x = -w / 2 + 5; x < w / 2 - 3; x += 7) ctx.fillRect(x, -h / 2 + 4, 2, h - 8);
            if (kind === "rug") {
              ctx.fillStyle = "rgba(48,37,30,.28)";
              roundedRectPath(ctx, -w / 2 + 7, -h / 2 + 6, w - 14, h - 12, 2);
              ctx.fill();
            }
          } else if (["stain", "mold", "spill", "blood_smear", "oil_stain"].includes(kind)) {
            const color = kind === "blood_smear" ? "rgba(92,28,26,.62)" :
              kind === "mold" ? "rgba(45,69,49,.42)" :
              kind === "spill" ? "rgba(99,83,51,.38)" :
              kind === "oil_stain" ? "rgba(20,24,22,.62)" :
              "rgba(58,48,37,.34)";
            ctx.fillStyle = color;
            ctx.beginPath();
            ctx.ellipse(0, 0, 15 + tone * 10, 6 + tone * 5, -0.25, 0, Math.PI * 2);
            ctx.ellipse(8, -3, 7 + tone * 4, 4, 0.2, 0, Math.PI * 2);
            ctx.fill();
          } else if (["papers", "files", "books", "trash", "glass", "cardboard", "tray", "coffee_cup"].includes(kind)) {
            const pieces = kind === "trash" || kind === "glass" ? 7 : 4;
            for (let i = 0; i < pieces; i += 1) {
              const x = -10 + ((i * 13 + variant * 5) % 22);
              const y = -7 + ((i * 9 + variant * 3) % 15);
              ctx.save();
              ctx.translate(x, y);
              ctx.rotate((i - 2) * 0.24);
              ctx.fillStyle = kind === "glass" ? "rgba(129,171,176,.45)" :
                kind === "trash" ? ["#615548", "#4f5a50", "#756b56"][i % 3] :
                kind === "books" ? ["#765147", "#536653", "#6a6077"][i % 3] :
                kind === "tray" ? "#777e79" :
                kind === "coffee_cup" ? "#d0c7ac" :
                kind === "cardboard" ? "#8a704e" : "#c8c0a5";
              ctx.fillRect(-4, -3, kind === "glass" ? 5 : 9, kind === "glass" ? 3 : 6);
              ctx.restore();
            }
          } else if (kind === "couch") {
            ctx.fillStyle = "#292722";
            roundedRectPath(ctx, -21, -11, 42, 23, 5);
            ctx.fill();
            ctx.fillStyle = cloth;
            roundedRectPath(ctx, -20, -10, 40, 19, 4);
            ctx.fill();
            ctx.fillStyle = "rgba(220,205,171,.1)";
            ctx.fillRect(-17, -8, 34, 3);
            ctx.strokeStyle = "rgba(38,31,27,.55)";
            ctx.strokeRect(-14, -5, 13, 11);
            ctx.strokeRect(1, -5, 13, 11);
            ctx.fillStyle = "#4a3d34";
            ctx.fillRect(-23, -8, 4, 17);
            ctx.fillRect(19, -8, 4, 17);
          } else if (["armchair", "chair", "office_chair", "waiting_chair"].includes(kind)) {
            const office = kind === "office_chair";
            ctx.fillStyle = office ? "#343b39" : cloth;
            roundedRectPath(ctx, -9, -10, 18, 18, 4);
            ctx.fill();
            ctx.fillStyle = "rgba(224,216,190,.1)";
            ctx.fillRect(-6, -7, 12, 3);
            ctx.strokeStyle = office ? "#656e69" : "#3c332b";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.moveTo(-6, 7);
            ctx.lineTo(-8, 14);
            ...
            ctx.stroke();
          } else if (["coffee_table", "nightstand", "radio"].includes(kind)) {
            const w = kind === "coffee_table" ? 28 : 18;
            const h = kind === "coffee_table" ? 15 : 17;
            ctx.fillStyle = "#30261e";
            roundedRectPath(ctx, -w / 2 - 1, -h / 2 - 1, w + 2, h + 2, 2);
            ctx.fill();
            ctx.fillStyle = kind === "radio" ? "#3d4643" : wood;
            roundedRectPath(ctx, -w / 2, -h / 2, w, h, 2);
            ctx.fill();
            ctx.fillStyle = "rgba(225,208,173,.12)";
            ctx.fillRect(-w / 2 + 2, -h / 2 + 2, w - 4, 3);
            if (kind === "radio") {
              ctx.fillStyle = "#151b19";
              ctx.fillRect(-6, -5, 12, 7);
              ctx.fillStyle = "#a49455";
          ...
          } else if (["bed", "mattress", "bunk"].includes(kind)) {
            const bunk = kind === "bunk";
            ctx.fillStyle = bunk ? "#343b38" : "#3d352e";
            roundedRectPath(ctx, -19, -11, 38, 22, 4);
            ctx.fill();
            ctx.fillStyle = kind === "mattress" ? "#b7b09a" : "#c6c0aa";
            roundedRectPath(ctx, -17, -9, 34, 18, 4);
            ctx.fill();
            ctx.fillStyle = "#ded7be";
            roundedRectPath(ctx, -15, -7, 10, 7, 2);
            ctx.fill();
            ctx.fillStyle = cloth;
            ctx.fillRect(-3, -7, 18, 14);
            if (bunk) {
              ctx.strokeStyle = "#737d77";
              ctx.lineWidth = 2;
          ...
          } else if (["bookshelf", "fallen_shelf"].includes(kind)) {
            ctx.fillStyle = "#2b241d";
            ctx.fillRect(-17, -7, 34, 14);
            ctx.fillStyle = wood;
            ctx.fillRect(-16, -6, 32, 12);
            ctx.fillStyle = "#282924";
            ctx.fillRect(-14, -2, 28, 2);
            ["#7b5147", "#516954", "#8c744c", "#59657a"].forEach((color, i) => {
              ctx.fillStyle = color;
              ctx.fillRect(-13 + i * 7, -5 + (i % 2) * 6, 5, 5);
            });
            if (kind === "fallen_shelf") {
              ctx.fillStyle = "#8c744c";
              ctx.fillRect(12, 8, 8, 5);
              ctx.fillStyle = "#59657a";
              ctx.fillRect(-17, 8, 7, 4);
            }
          } else if (["lamp", "iv_stand"].includes(kind)) {
            ctx.strokeStyle = kind === "iv_stand" ? "#8a9690" : "#4d4539";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.moveTo(0, -13);
            ctx.lineTo(0, 11);
            ...
            ctx.stroke();
            ctx.fillStyle = kind === "iv_stand" ? "rgba(191,214,203,.55)" : "#b9a66c";
            ctx.beginPath();
            ctx.arc(kind === "iv_stand" ? -5 : 0, kind === "iv_stand" ? -4 : -14, kind === "iv_stand" ? 3 : 7, 0, Math.PI * 2);
            ctx.fill();
          } else if (kind === "dead_plant") {
            ctx.fillStyle = "#68513a";
            roundedRectPath(ctx, -7, 4, 14, 10, 3);
            ctx.fill();
            ctx.strokeStyle = "#596040";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.moveTo(0, 4);
            ctx.lineTo(-2, -11);
            ...
            ctx.lineTo(8, -8);
            ctx.stroke();
          } else if (["suitcase", "boxes", "evidence_box"].includes(kind)) {
            const count = kind === "boxes" ? 3 : 1;
            for (let i = 0; i < count; i += 1) {
              const x = i === 0 ? -9 : i === 1 ? 4 : -2;
              const y = i === 2 ? -8 : 0;
              ctx.fillStyle = kind === "suitcase" ? "#554d43" : kind === "evidence_box" ? "#b8af91" : "#826848";
              roundedRectPath(ctx, x - 8, y - 6, 16, 12, 2);
              ctx.fill();
              ctx.strokeStyle = "rgba(43,34,27,.55)";
              ctx.strokeRect(x - 6.5, y - 4.5, 13, 9);
            }
          } else if (["shopping_cart", "wheelchair", "hand_truck"].includes(kind)) {
            ctx.strokeStyle = "#78837e";
            ctx.lineWidth = 2;
            ctx.beginPath();
            ...
            roundedRectPath(ctx, -20, -10, 40, 20, 3);
            ctx.fill();
            ctx.fillStyle = "rgba(214,225,214,.22)";
            ...
            ctx.lineTo(22, 15);
            ctx.stroke();
          } else if (["bench", "waiting_bench"].includes(kind)) {
            ctx.fillStyle = kind === "waiting_bench" ? "#54615e" : wood;
            roundedRectPath(ctx, -20, -7, 40, 14, 3);
            ctx.fill();
            ctx.fillStyle = "rgba(226,216,188,.1)";
            ctx.fillRect(-17, -5, 34, 3);
            ctx.fillStyle = "#343a37";
            ctx.fillRect(-15, 7, 4, 7);
            ctx.fillRect(11, 7, 4, 7);
          } else if (kind === "pallet") {
            ctx.fillStyle = "#6d5538";
            for (let y = -9; y <= 7; y += 8) ctx.fillRect(-18, y, 36, 5);
            ctx.fillStyle = "#3f3428";
            ctx.fillRect(-14, -11, 4, 24);
            ctx.fillRect(10, -11, 4, 24);
          } else if (kind === "barrel") {
            ctx.fillStyle = metal;
            roundedRectPath(ctx, -10, -13, 20, 26, 6);
            ctx.fill();
            ctx.fillStyle = "rgba(220,225,208,.12)";
            ctx.fillRect(-7, -10, 5, 20);
            ctx.strokeStyle = "#303735";
            ctx.strokeRect(-9.5, -8, 19, 4);
            ctx.strokeRect(-9.5, 5, 19, 4);
          } else if (kind === "tire") {
            ctx.fillStyle = "#202322";
            ctx.beginPath();
            ctx.arc(0, 0, 12, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#555b57";
            ctx.beginPath();
            ctx.arc(0, 0, 5, 0, Math.PI * 2);
            ctx.fill();
          } else if (kind === "toilet") {
            ctx.fillStyle = "#c5c8bb";
            roundedRectPath(ctx, -8, -12, 16, 11, 3);
            ctx.fill();
            ctx.beginPath();
            ctx.ellipse(0, 5, 11, 9, 0, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = "#575e59";
            ctx.beginPath();
            ctx.ellipse(0, 5, 6, 4, 0, 0, Math.PI * 2);
            ctx.fill();
          } else if (["graffiti", "picture_frame", "sign"].includes(kind)) {
            ctx.fillStyle = kind === "picture_frame" ? "#513e30" : kind === "sign" ? "#6f704f" : "rgba(94,60,52,.7)";
            roundedRectPath(ctx, -14, -8, 28, 16, 2);
            ctx.fill();
            ctx.fillStyle = kind === "picture_frame" ? "#8a8271" : "#d1c79f";
            if (kind === "graffiti") {
              ctx.font = "900 10px system-ui, sans-serif";
              ctx.textAlign = "center";
              ctx.textBaseline = "middle";
              ctx.fillText(variant % 2 ? "NO" : "RUN", 0, 1);
            } else {
              ctx.fillRect(-10, -5, 20, 10);
            }
          } else if (kind === "mop_bucket") {
            ctx.fillStyle = "#9c8a4d";
            roundedRectPath(ctx, -9, -7, 18, 15, 4);
            ctx.fill();
            ctx.fillStyle = "#303633";
            ctx.fillRect(-7, -4, 14, 3);
            ctx.strokeStyle = "#7d8782";
            ctx.beginPath();
            ctx.moveTo(5, -5);
            ctx.lineTo(13, -18);
            ctx.stroke();
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(-6, 0, 9, 0.2, Math.PI * 1.8);
            ctx.arc(7, 1, 8, Math.PI * 1.1, Math.PI * 2.7);
            ctx.stroke();
          }
